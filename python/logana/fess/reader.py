...
class SearchLogReader(ranking.Reader):
    ...
    @staticmethod
    def _create_impression(data: Dict[str, Any]) -> Dict[str, Dict[str, Any]]:
        results: Dict[str, Any] = {}
        for i, doc in enumerate(data.get('documents') if 'documents' in data else []):
            results[f'doc_{i+1}'] = {
                'id': doc.get('doc_id'),
                'keyword': {
        ...
        for log_file in self._log_files:
            with self._open(log_file) as f:
                for line in f.readlines():
                    ...
                    event_type: str = log_obj.get('event_type')
                    if event_type == 'log':
                        ipression: Dict[str, Dict[str, Any]] = self._create_impression(
                            log_obj)
                        impressions[ipression['request']
                                    ['id']['query']] = ipression
