    ...
    def get_column(self) -> FeatureColumn:
        if self.column_type == 'embedding':
            categorical_column_ = tf.feature_column.categorical_column_with_vocabulary_file(
                key=self.name,
                vocabulary_file=self.dictionary)
...
class TfRankingModelConfig:
    ...
    hidden_layer_dims: List[int] = dataclasses.field(
...
def _get_context_feature_columns(config: TfRankingModelConfig) -> Dict[str, FeatureColumn]:
...
def _get_example_feature_columns(config: TfRankingModelConfig) -> Dict[str, FeatureColumn]:
    columns: Dict[str, FeatureColumn] = {}
...
def _input_fn(config: TfRankingModelConfig, is_train: bool = True) -> Tuple[Any]:
    context_feature_spec: Dict[str, FeatureColumn] = tf.feature_column.make_parse_example_spec(
    ...
    example_feature_spec: Dict[str, FeatureColumn] = tf.feature_column.make_parse_example_spec(
        example_columns)
    ...
    features: Any = tf.compat.v1.data.make_one_shot_iterator(
        dataset).get_next()
...
def _make_score_fn(config: TfRankingModelConfig) -> Callable:
    def _score_fn(context_features, group_features, mode, params, _config):
        ...
        logits = tf.compat.v1.layers.dense(
            cur_layer, units=config.group_size)
        return logits
...
def run_train(config: TfRankingModelConfig) -> Tuple[Any]:
    ...
    def train_input_fn() -> Callable:
        return _input_fn(config)

    def eval_input_fn() -> Callable:
...
def save_model(ranker: tf.estimator.Estimator, config: TfRankingModelConfig) -> str:
    return ranker.export_saved_model(f'{config.model_path}',
                                     _make_serving_input_fn(config),
                                     checkpoint_path=f'{config.model_path}/model.ckpt-{config.num_train_steps}').decode('utf-8')
